...
export function useTotalAumGrowth(asOfDate?: string, reportPeriod?: string, totalReportedReportPeriod?: string) {
  const params = new URLSearchParams();
  if (asOfDate) params.set("asOfDate", asOfDate);
  if (reportPeriod) params.set("reportPeriod", reportPeriod);
  if (totalReportedReportPeriod) params.set("totalReportedReportPeriod", totalReportedReportPeriod);
