...
export function DailyDataTable() {
  ...
  useRegisterExport(() => ({
    ...
    rows: rows.map((r) => ({
      ...
      "Indian Stocks + Cash": r.indianStocksAndCash,
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm text-muted-foreground">
        ...
        got a live DHAN close, versus debt/foreign/cash-equivalent lines that never could. Total Holdings − Debt
        Instruments − Foreign Holdings = Indian Stocks + Cash; Coverage % = Live Considered ÷ that figure. Rows below
        85% are tinted amber, below 80% red — the same 80% floor that lights up the Overview banner. Updated
        automatically each trading day shortly after market close.
      </p>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">Indian Stocks + Cash</TableHead>
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r.snapshotDate} className={rowClassName(r.coveragePct)}>
                ...
                <CountCell value={r.indianStocksAndCash} />
